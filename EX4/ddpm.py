...
class Diffusion:
    ...
    def p_mean_std(self, model, x_t, t):
        ...
        mean = (1 / torch.sqrt(alpha)) * (x_t - ((1 - alpha) / torch.sqrt(1 - alpha_bar)) * predicted_noise) # HINT: calculate the mean of the distribution p(x_{t-1} | x_t). See Eq. 11 in the ddpm paper at page 4
        std = torch.sqrt(beta)
